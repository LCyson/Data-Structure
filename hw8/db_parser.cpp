...
Review* DBParser::parseReview(istream& is, string prodname)
{
  ...
  if( is.fail() ){
    error_ = true;
    errorMsg_ = "Unable to read user's name";
  }
  is >> date;
  if( is.fail() ){
    error_ = true;
  ...
  if(error_){
    ...
    return makeReview(prodname, rating, date, review_text, u);
  }

}

Review* DBParser::makeReview(string& prodname, int& rating, string& date, string& review_text, string& user)
{
  return new Review(prodname, rating, date, review_text, user);
}
