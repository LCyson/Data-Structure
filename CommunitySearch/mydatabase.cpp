...
Mydatabase::~Mydatabase()
{
	...
	for (vector<User*>::iterator it = for_delete_u.begin(); it != for_delete_u.end(); ++it)
	{
		delete *it;
	}
	for (vector<Review*>::iterator i = for_delete_reviews.begin(); i != for_delete_reviews.end(); ++i)
	{
		delete *i;
	}
}
...
void Mydatabase::addUser(User* u)
{
	...
	for_delete_u.push_back(u);
}

void Mydatabase::addReview(Review* r)
{
	set<string> keyword = r->keywords();
	vector<string> keywords;

	for (std::set<string>::iterator i = keyword.begin(); i != keyword.end(); ++i)
	{
		keywords.push_back(*i);
	}

	vector<Product*> temp = search( keywords, 0 );

	temp[0]->storeReview(r);

	for_delete_reviews.push_back(r);

	for (unsigned int i = 0; i < for_delete_u.size(); ++i)
	{
		if ( for_delete_u[i]->getName() == r->getUName() )
		{
			for_delete_u[i]->storeReview(r);
		}
	}
}
...
void Mydatabase::dump(std::ostream& ofile)
{
	...
	vector<Review*>::iterator t;
	for (t = for_delete_reviews.begin(); t != for_delete_reviews.end(); ++t)
	{
		(*t)->dump( ofile );
	}
...
std::vector<User*>& Mydatabase::getUser()
{
	return for_delete_u;
}

std::set<Product*>& Mydatabase::getProduct()
{
	return for_delete;
}